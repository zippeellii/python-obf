...
def generate_key():
	return ''.join([random.choice('abcdefghijklmnopqrstu') for _ in xrange(10)])
...
def _encrypt_char(c, k):
	return chr(((ord(c)-97 + ord(k)-97)%26)+97) if c in 'abcdefghijklmnopqrstuvwxyz' else c
...
def _decrypt_char(c, k):
	lol = chr(((ord(c)-97 - ord(k)+97)%26)+97) if c in 'abcdefghijklmnopqrstuvwxyz' else str(c)
	print 'C:', c
	print 'K:', k
